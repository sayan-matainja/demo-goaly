...
$(document).ready(function () {
                ...
                if (responses.userFavteams.length > 0) {
                    ...
                    $.each(responses.userFavteams, function (index, response) {
                        ...
                        var editIcon =
                            ...
                            "/team/details/" +
                            response.id +
                            '" id="clubDetail"><i class="fa fa-external-link" aria-hidden="true" style="font-size: 20px; color: rgb(0, 153, 204);"> </i></i></a>';
                        var deleteIcon =
                            '<a class="delete-icon" data-id="' +
                            response.table_id +
                            '" id="deleteIcn"><i class="fa fa-trash" style="font-size: 22px; color: red; padding: 6px;"></i></a>';
                        var badgeImage =
                            '<img src="' +
                            response.badge +
                            '" alt="Badge" style="height: 30px; width: auto;" id="clubImg">';

                        row.append("<td >" + badgeImage + "</td>");
                        row.append(
                            '<td id="clubName">' + response.name + "</td>"
                        );
                    ...
                    $("#UserFavourite").append(
                        '<div><img src="assets/img/no-club.png" alt="Club" style="height:69px; margin-right: 25px"></div>'
                    );
                    tableBody.append(
    ...
    $j(document).on("click", ".delete-icon", function (event) {
        ...
        Swal.fire({
            title:
                '<span id="popUpTitle">' + translations.team_remove + "</span>",
            icon: "warning",
            showCancelButton: true,
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.players.length === 0) {
                    var noPlayerRow =
                        '<div style="display: flex; line-height: 100px; align-items: center; justify-content: center;"><span style="color: red; font-size: 14px; font-weight: 100; letter-spacing: 1px;" id="noPlayerTxt"> ' +
                        translations.no_players +
                        "</span></div>";
                ...
                var tableHeading =
                    ...
                    "</th>" +
                    (data.status === "winner"
                        ? '<th id="pointTitle">' + translations.Points + "</th>"
                        : '<th id="pointTitle">' +
                ...
                $.each(data.players, function (index, player) {
                    var playerName = !isEmpty(player.name)
                        ? player.name
                        : "xxxxxxx" + player.msisdn.slice(-4);
                    var row =
                        '<tr class="wpos">' +
                        '<td id="number">' +
                        (index + 1) +
                        "</td>" +
                        '<td id="user"><a class="contestants_answer" href="#" data-userid="' +
                        player.user_id +
                        '" data-predid="' +
                        player.pred_id +
    ...
    $(document).on("click", ".contestants_answer", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#cotestent").html(
                    '<div id="cotestent" style="width: 100%;"><span><img id="User Image" src="' +
                        (data.user_image !== null
                            ? data.user_image
                            : base_url +
                              "/images/leaderboard/user_no_image.png") +
                        '" style="height: 22px; width: auto; float: left; margin-top: 20px; margin-left: 4px; margin-right: 9px;" alt="cotestent"></span><span style="float: left;"><h2><span id="playtime"> ' +
                        data.created_at +
                        "</span></h2></span></div>"
                );
                var tableHeading =
                ...
                $.each(data.questionAnswers, function (index, question) {
                    var translatedQuestion =
                        translations.question?.[question.question.trim()] ||
                        question.question;
                    var row =
                        '<tr class="wpos">' +
                        '<td id="number">' +
                    ...
                    if (index === 0) {
                        row +=
                            '<img src="' +
                            question.homelogo +
                            '" style="height: 15px; width: auto;" alt="Home Team">';
                        row +=
                            "<span> " +
                            question.home_score +
                            " - " +
                        ...
                        row +=
                            '<img src="' +
                            question.awaylogo +
                            '" style="height: 15px; width: auto;" alt="Away Team">';
                    } else {
    ...
    $dropdownMenu.on("click", "li", function () {
        ...
        if (leagueLogo) {
            $dropdownButton.find(".dropdown-toggle").html(`
              <img src="${leagueLogo}" class="selected-image" alt="League Logo" /> ${leagueName}
                ...
                if (response.competitions.length !== 0) {
                    $.each(
                        ...
                        function (index, competition) {
                            var buttonText =
                                ...
                                competition.prediction_end_time
                                    ? translations.who_play
                                    : translations.prediction_result;
                            var contestLink = response.userId
                                ? "/contest/detail/" + competition.id
                                : "/login";
                            contest_html +=
                                '<div class="contest" style="background: ' +
                                (competition.venue_image
                                    ? "url(" + competition.venue_image + ") "
                                    : "url(/assets/img/stadium.png) ") +
                                'no-repeat, linear-gradient(to bottom, rgb(28, 51, 8), black); background-size: 100% 100%, cover; background-position: 0 0;">';
                            ...
                            contest_html +=
                                '<a href="/team/details/' +
                                competition.home_team_id +
                                '">';
                            contest_html +=
                                '<div class="logo"><img src="' +
                                competition.home_team_logo +
                                '" id="hClubImg" alt="Home Team"></div>';
                            contest_html += "</a>";
                            contest_html +=
                                '<h5 class="mb-0" id="hClubName">' +
                            ...
                            contest_html +=
                                '<div class="logo" style="width: 83px; height: 61px; background: white; border-radius: 53px; padding: 7px 5px; margin: auto auto 10px;">';
                            contest_html +=
                                '<img id="leagueImg"src="' +
                                base_url +
                                "/assets/img/league/" +
                                competition.league_logo +
                                '" alt="League Logo" style="width: 50px;margin-top:5px" alt="Away Team Logo">';
                            contest_html += "</div>";
                            contest_html +=
                                '<div class="date p-1" id="matchDay">' +
                                competition.match_time_bar +
                                "<br>" +
                                competition.match_time +
                                "</div>";
                            contest_html +=
                                '<div class="place p-1" id="location">' +
                                competition.venue +
                                "</div>";
                            contest_html += "</div>";
                            contest_html += "</div>";
                            ...
                            contest_html +=
                                '<a href="/team/details/' +
                                competition.away_team_id +
                                '">';
                            contest_html +=
                                '<div class="logo"><img id="aClubImg"src="' +
                                competition.away_team_logo +
                                '" alt="Away Team"></div>';
                            contest_html += "</a>";
                            contest_html +=
                                '<h5 class="mb-0" id="aClubName">' +
                                competition.away_team_name +
                                "</h5>";
                            contest_html += "</div>";
                            contest_html += "</div>";
                            var loginButton =
                                '<a href="/login"><button type="button" id="playBtn" class="btn bg-green p-2 w-100 my-2 text-white" style="font-size: 12pt;"><strong>' +
                                translations.lets_play +
                                "</strong></button></a>";
                            var letsPlayButton =
                                '<a href="/contest/detail/' +
                                competition.id +
                                '"><button id="playBtn" type="button" class="btn bg-green p-2 w-100 my-2 text-white" style="font-size: 12pt;"><strong>' +
                                translations.lets_play +
                                "</strong></button></a>";
                            var matchResultButton =
                                '<a href="/match/details/' +
                                competition.fixture_id +
                                '"><button type="button" id="matchResultBtn" class="btn bg-grey p-2 w-100 my-2 text-white" style="font-size: 12pt;"><strong>' +
                                translations.match_result +
                                "</strong></button></a>";
                            var comingSoonButton =
                                '<a href="javascript:void(0)"><button type="button" class="btn bg-green p-2 w-100 my-2 text-white" style="font-size: 12pt;"><strong>' +
                                translations.coming_soon +
                                "</strong></button></a>";

                            if (
                                response.current_time >=
                                    competition.prediction_start_time &&
                                response.current_time <=
                                    competition.prediction_end_time &&
                                ...
                                competition.prediction_end_time <
                                    response.current_time ||
                                competition.match_time < response.current_time
                            ) {
                            ...
                            contest_html +=
                                '<div class="w-50 p-1 bg-whitesmoke border-right" style="border-top-left-radius: 10px;" id="startText">' +
                                translations.start +
                                ': <br> <strong id="startDate">' +
                                competition.prediction_start_time +
                                "</strong></div>";
                            contest_html +=
                                '<div class="w-50 p-1 bg-whitesmoke" style="border-top-right-radius: 10px;" id="endTxt">' +
                                translations.end +
                                ': <br> <strong id="endDate">' +
                                competition.prediction_end_time +
                                "</strong></div>";
                            contest_html += "</div>";
                            contest_html +=
                                '<div class="w-100 p-2 text-white" style="border: 1px solid whitesmoke; border-radius: 0 0 10px 10px;" id="point">' +
                                translations.total_point_win +
                                ": <b>100</b></div>";

                            contest_html +=
                                '<a href="#" class="show-contestants-link" data-id="' +
                                competition.id +
                                '" data-toggle="modal" data-target="#UserPlay">';
                            contest_html +=
                                '<button type="button" class="btn btn-lg btn-purple mt-2" id="seeWhoPlayBtn">' +
                                buttonText +
                                "</button>";
                            contest_html += "</a>";
                            contest_html += "</div>";
                    ...
                    $("#contest").html(
                        '<div align="center"><img src="' +
                            base_url +
                            '/assets/img/detail-match/icon-menu/no-Data-Found.png" class="no-data-img" alt="No Data Found"></div>'
                    );
                }
            },
