...
<div class="container pd-x-0">
    <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                <li class="breadcrumb-item"><h5>Edit prize</h5></li>
            </ol>
        </nav>
    </div>
    ...
    <form class="needs-validation" action="{{url('/admin/prize/update')}}" method="POST" enctype="multipart/form-data" data-parsley-validate novalidate>
        @csrf
        <input type="hidden" name="prize_id" value="{{$dtl->prize_id}}">
        <div class="row row-xs">
            <div class="col-lg-5">
                <div class="form-group row label-match">
                    <label class="col-sm-4 col-form-label">Reward text:</label>
                    <input type="text" name="reward_text" class="form-control" value="{{$dtl->reward_text}}" required>
                </div>
                <div class="form-group row label-match">
                    <label class="col-sm-4 col-form-label">Point to exchanged:</label>
                    <input type="text" name="points" class="form-control" value="{{$dtl->points}}" required>
                </div>
                <div class="form-group row label-match">
                    <label class="col-sm-4 col-form-label">Quantity:</label>
                    <input type="text" name="quantity" class="form-control" value="{{$dtl->quantity}}" required>
                </div>
