  ...
  <div id="myCarousel" class="carousel slide" data-ride="carousel">
    ...
    <div class="carousel-inner">
      <div class="item active">
        <img  id="rewardImg" src="./images/bannerImage/demo-goaly-banner-1.jpg" alt="" style="width:100%;">
      </div>

      <div class="item">
        <img id="rewardImg" src="./images/bannerImage/demo-goaly-banner-2.png" alt="" style="width:100%;">
      </div>
...
<!-- slider end -->
            ...
            <div class="timeline">
                ...
                <div class="timeline-group-header {{ $item['news'] === 'Transfer News' ? 'header-transfer' : '' }} clearfix">

                        <span class="float-left">
                            <span class="icon-transfer mr-1" id="newsTypeIcn"></span> <strong id="newsType">{{ trans('lang.' . $item['news']) }}</strong>
                        </span>
                        <span class="float-right text-right"><a class="seeAllLink" href="javascript:void(0)" data-newsType="{{$item['news']}}" id="seeAllLink">{{ trans('lang.See All') }}</a></span>
                    </div>
                    <div class="timeline-group-content media">
                        <div class="media-left p-0">
                            <div class="thumbail-news">
                             <img id="newsImg" src="{{ $item['urlToImage'] }}"                     onerror="this.onerror=null; this.src='{{ asset('images/newsImage/news-default.jpg') }}';" 
                                 alt="">
                            </div>
                        </div>
...
<!----------------------for showing Predictions/competitions -------------------->
                    ...
                    <div class="timeline-group-header header-match clearfix">
                        <span class="float-left">
                            <span class="icon-ball mr-1" id="matchIcn"></span> <strong id="matchTxt">{{ trans('lang.Match') }}</strong>
                        </span>
                         @if($today>= $item['prediction_start_time'] && $today<= $item['prediction_end_time']  && $today<$item['match_time'])  
                        <span class="timeline-label blue" id="matchStatus">{{ trans('lang.Ongoing Prediction') }} </span>
                        @elseif($today>$item['prediction_end_time'] || $today>$item['match_time']  )
                        <span class="timeline-label red" id="matchStatus">{{ trans('lang.End Match') }}</span>

                        @else
                        <span class="timeline-label blue" id="matchStatus">{{ trans('lang.Coming Soon') }}</span>
                        @endif
                    </div>
                    <div class="match-area">
                        <div class="row small mb-1">
                            <div class="col-xs-6">
                                <p class="m-0 text-left" id="league">{{ $item['league_name'] ?? 'N/A' }}</p>
                            </div>
                            <div class="col-xs-6">
                                <p class="m-0 text-right" id="matchDate">{{$item['match_time'] ?? 'N/A'}}</p>
                            </div>
                        </div>
                                ...
                                <div class="club-box club-l">
                                    <div class="club-pict">
                                        <a href="/team/details/{{$item['home_team_id']}}">
                                            <img id="hClubImg" src="{{$item['home_team_logo'] ?? 'N/A'}}" alt="club">
                                        </a>
                                    </div>
                                    <div class="club-name" >
                                        <span id="hClubName">{{ isset($item['home_short_code']) ? $item['home_short_code'] : strtoupper(substr($item['home_team_name'], 0, 3)) }}
                                    </span>
                                       
                                        <span class="match-result" id="hClubScore">{{$item['home_team_score'] ?? '-' }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-xs-5">
                                <div class="club-box club-r">
                                    <div class="club-name">
                                        ...
                                        <span class="match-result" id="aClubScore">{{ $item['away_team_score'] ?? '-' }}</span>
                                    </div>
                                    <div class="club-pict">
                                        <a href="/team/details/{{$item['away_team_id']}}">
                                        <img id="aClubImg" src="{{$item['away_team_logo'] ?? 'N/A'}}" alt="club">
                                        </a>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="text-center">
                          @if($today>= $item['prediction_start_time'] && $today<= $item['prediction_end_time'] && $today<$item['match_time']) 
                                @if(session('UserId')) 
                                <a href="/contest/detail/{{$item['id']}}" class="btn bg-green p-1 w-50 text-white mx-auto" id="joinBtn">{{ trans('lang.Join') }}</a>
                                @else
                                <a href="{{route('logined')}}" class="btn bg-green p-1 w-50 text-white mx-auto" id="joinBtn">{{ trans('lang.Join') }}</a>
                                @endif
                           @elseif($today>$item['prediction_end_time'] || $today>$item['match_time'] )

                                <a href="/match/details/{{$item['fixture_id']}}" class="btn btn-purple p-1 w-50 text-white mx-auto" id="seeResultBtn">{{ trans('lang.See Result') }}</a>
                           @else
...
<!-- upcomming match start -->
            ...
            <!-- @if(!empty($response['matches']))
              @foreach($response['matches'] as $i=> $match) 
              
              <div class="timeline-group" {{ $i > 2 ? 'loading=lazy' : '' }}>
                    <div class="timeline-group-header header-match clearfix">
                        <span class="float-left">
                            <span class="icon-ball mr-1" id="matchIcn"></span> <strong id="matchTxt">{{ trans('lang.Match') }}</strong>
                        </span>
                        <span class="timeline-label" id="matchTxt">{{ trans('lang.'.$match['match_status']) }}</span>
                    </div>
                    <div class="match-area">
                        <div class="row small mb-1">
                            <div class="col-xs-6">
                                <p class="m-0 text-left" id="league">{{$match['league']}}</p>
                            </div>
                            <div class="col-xs-6">
                                <p class="m-0 text-right" id="matchDate">{{$match['date_time']}}</p>
                            </div>
                        </div>
                        <div class="row d-flex ais-center mb-1 min-gutter">
                            <div class="col-xs-5">
                                <div class="club-box club-l">
                                    <div class="club-pict">
                                        <a href="/team/details/{{$match['homeTeam_id']}}">
                                        <img src="{{$match['homeTeam_image']}}" alt="club" id="hClubImg">
                                        </a>
                                    </div>
                                    <div class="club-name">
                                        <span id="hClubName">
                                       {{ isset($match['home_short_code']) ? $match['home_short_code'] : strtoupper(substr($match['homeTeam_name'], 0, 3)) }}
                                   </span>
                                        <span class="match-result" id="hClubScore">{{$match['localteam_score']}}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-xs-2 text-center" id="vs">VS</div>
                            <div class="col-xs-5">
                                <div class="club-box club-r">
                                    <div class="club-name">
                                        <span id="aClubName">
                                        {{ isset($match['away_short_code']) ? $match['away_short_code'] : strtoupper(substr($match['awayTeam_name'], 0, 3)) }}
                                    </span>
                    
                                        <span class="match-result" id="aClubScore">{{$match['visitorteam_score']}}</span>
                                    </div>
                                    <div class="club-pict">
                                        <a href="/team/details/{{$match['awayTeam_id']}}">                                  
                                        <img src="{{$match['awayTeam_image']}}" alt="club" id="aClubImg">
                                        </a>
                                    </div>
                                </div>
                            </div>
  ...
  <div class="modal-dialog" style="margin-left: 23px">
    ...
    <div class="modal-content" style="margin-left:-14px;margin-top:67px">
      ...
      @if(!empty($details))
            ...
            <button  id ="saveBtn" type="button" class="btn btn-success btn-lg btn-block" style="background: rgb(22, 80, 14);color:white">{{ trans('lang.Save') }}</button>
              <button  id ="closeBtn" type="button" class="btn btn-secondary btn-lg btn-block" data-dismiss="modal"  style="background:rgb(215, 13, 78); color:white">{{ trans('lang.Close') }}</button>
            </div>
